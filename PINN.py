...
class PINN_Methods(object):
    ...
    def solve_PINN_on_FEM_dofs(self, model, mu0, mu1, plot_solution=False):
        ...
        def evaluate_all_components(x, y):
            ...
            if key not in cache:
                ...
                with torch.inference_mode():
                    values = model(point)[0].detach().cpu().numpy()
        ...
        def u_x_function(x, y, z):
            return float(evaluate_all_components(x, y)[0])

        def u_y_function(x, y, z):
            return float(evaluate_all_components(x, y)[1])

        def p_function(x, y, z):
            return float(evaluate_all_components(x, y)[2])
