    ...
    def build_f_components(self,mu1:float):
        ...
        if not VALIDATE_FOM:
            ...
            def f_x_function(x: float, y: float, z: float):
                ...
                ux_x = A * pi * sx * cx * sy * cy
                ux_y = 0.5 * A * pi * sx**2 * (cy**2 - sy**2)

                lap_ux = A * pi**2 * (cx**2 - 3.0 * sx**2) * sy * cy

                p_x = A * pi * cx * sy

                return -mu0 * lap_ux + ux * ux_x + uy * ux_y + p_x
            ...
            def f_y_function(x: float, y: float, z: float):
                ...
                uy_x = -0.5 * A * pi * sy**2 * (cx**2 - sx**2)
                uy_y = -A * pi * sy * cy * sx * cx

                lap_uy = A * pi**2 * (3.0 * sy**2 - cy**2) * sx * cx

                p_y = A * pi * sx * cy

                return -mu0 * lap_uy + ux * uy_x + uy * uy_y + p_y
